...
const PreSaleYardMint = () => {
  ...
  const handlePresaleYardMint = async () => {
    if (connectStatus && coinbase) {
      const isWhitelistedForPresell = await seeds_controller.isWhitelistedForPresell(coinbase)
      if (isWhitelistedForPresell) {
        setOpen(true)
        try {
          let value =  new BigNumber(`${tokenNum * 0.15 * 10 ** 18}`);
          const tx = await seeds_controller.buyYardUsingBNB(value)
          if (tx?.status) {
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <PercentageButton percent={`${rewardData.yardCarbon ? rewardData.yardCarbon / 10 : 0}`}/>
          <PercentageButton percent={`${rewardData.yardPearl ? rewardData.yardPearl / 10 : 0}`} />
          <PercentageButton percent={`${rewardData.yardRuby ? rewardData.yardRuby / 10 : 0}`} />
          <PercentageButton percent={`${rewardData.yardDiamond ? rewardData.yardDiamond / 10 : 0}`} />
        </Box>
        <Box
          sx={{
            display: "flex",
        ...
        <Box>
          ...
          >
            Whitelist Offer: {tokenNum * 0.15} BNB / Mint{" "}
          </Title>
        </Box>
      </Box>
      <ProgresModal
...
export default PreSaleYardMint;
